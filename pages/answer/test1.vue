<template>
	<view class="bg">
		<view class="answer-bg">
			...
			<view class="analysis">
				<swiper  class="swiper-text" :current="questionIndex"  @change="swiperChange">
					<swiper-item v-for="(item,index) in questionList" :key="index">
						<!-- 解析 -->
						<view v-if="item.additional.showAnalysis" class="test-analysis">
							...
							<view class="ttt" v-if="item.analysis" v-html="item.analysis"></view>
							<view class="update-text" v-else>暂无解析</view>
						</view>
					</swiper-item>
				</swiper>
			</view>
		...
		<view class="cu-bar tabbar bg-white shadow foot">
			...
			<view class="action">
				<button size="mini" type="primary" @click="tapSubmit()" data-target="modalCard">提交</button>
			</view>
		</view>
	</view>
</template>
...
<script>
	const data = require('utils/data2.json');
	export default {
		data() {
			return {
				hour: 0,
